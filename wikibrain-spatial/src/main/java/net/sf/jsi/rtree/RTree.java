...
public class RTree implements SpatialIndex, Serializable {
   ...
   * </ul></p>
   *
   * @see net.sf.jsi.SpatialIndex#init(java.util.Properties)
   */
  ...
  /**
   * @see net.sf.jsi.SpatialIndex#nearest(Point, gnu.trove.procedure.TIntProcedure, float)
   */
  ...
  /**
   * @see net.sf.jsi.SpatialIndex#nearestNUnsorted(Point, gnu.trove.procedure.TIntProcedure, int, float)
   */
  ...
  /**
   * @see net.sf.jsi.SpatialIndex#nearestN(Point, gnu.trove.procedure.TIntProcedure, int, float)
   */
  public void nearestN(Point p, TIntProcedure v, int count, float furthestDistance) {
     ...
     * Shilad's hack
     */
    public List<SpatialDistanceMetric.Neighbor> nearestN(Point p, int count, float furthestDistance) {
        PriorityQueue distanceQueue = new PriorityQueue(PriorityQueue.SORT_ORDER_DESCENDING);
        createNearestNDistanceQueue(p, count, distanceQueue, furthestDistance);
        distanceQueue.setSortOrder(PriorityQueue.SORT_ORDER_ASCENDING);

        List<SpatialDistanceMetric.Neighbor> result = new ArrayList<SpatialDistanceMetric.Neighbor>();
        while (distanceQueue.size() > 0) {
            result.add(new SpatialDistanceMetric.Neighbor(distanceQueue.getValue(), distanceQueue.getPriority()));
            distanceQueue.pop();
        }
        return result;
    }
  ...
  /**
   * @see net.sf.jsi.SpatialIndex#intersects(Rectangle, gnu.trove.procedure.TIntProcedure)
   */
  ...
  /**
   * @see net.sf.jsi.SpatialIndex#contains(Rectangle, gnu.trove.procedure.TIntProcedure)
   */
