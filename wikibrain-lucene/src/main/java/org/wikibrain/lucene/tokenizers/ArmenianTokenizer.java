 ...
 * @author Ari Weiland
 */
public class ArmenianTokenizer extends LanguageTokenizer {

    protected ArmenianTokenizer(Version version, TokenizerOptions tokenizerOptions, Language language) {
    ...
    public TokenStream getTokenStream(Tokenizer tokenizer, CharArraySet stemExclusionSet) {
        ...
        if (useStopWords)
            stream = new StopFilter(matchVersion, stream, ArmenianAnalyzer.getDefaultStopSet());
        if (useStem) {
