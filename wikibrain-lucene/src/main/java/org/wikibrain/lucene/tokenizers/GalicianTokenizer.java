 ...
 * @author Ari Weiland
 */
public class GalicianTokenizer extends LanguageTokenizer {

    protected GalicianTokenizer(Version version, TokenizerOptions tokenizerOptions, Language language) {
    ...
    public TokenStream getTokenStream(Tokenizer tokenizer, CharArraySet stemExclusionSet) {
        ...
        if (useStopWords)
            stream = new StopFilter(matchVersion, stream, GalicianAnalyzer.getDefaultStopSet());
        if (useStem) {
            if (!stemExclusionSet.isEmpty())
                stream = new SetKeywordMarkerFilter(stream, stemExclusionSet);
            stream = new GalicianMinimalStemFilter(stream);
        }
        return stream;
    }
}
