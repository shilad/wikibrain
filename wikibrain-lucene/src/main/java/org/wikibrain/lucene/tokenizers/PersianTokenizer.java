 ...
 * and is not currently added to LanguageTokenizer
 *
 * @author Ari Weiland
 */
public class PersianTokenizer extends LanguageTokenizer {

    protected PersianTokenizer(Version version, TokenizerOptions tokenizerOptions, Language language) {
    ...
    public TokenStream getTokenStream(Tokenizer tokenizer, CharArraySet stemExclusionSet) {
        ...
        if (useStopWords)
            stream = new StopFilter(matchVersion, stream, PersianAnalyzer.getDefaultStopSet());
        if (useStem) {
