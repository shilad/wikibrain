 ...
 * @author Ari Weiland
 */
public class RussianTokenizer extends LanguageTokenizer {

    protected RussianTokenizer(Version version, TokenizerOptions options, Language language) {
    ...
    public TokenStream getTokenStream(Tokenizer tokenizer, CharArraySet stemExclusionSet) {
        ...
        if (useStopWords)
            stream = new StopFilter(matchVersion, stream, RussianAnalyzer.getDefaultStopSet());
        if (useStem) {
