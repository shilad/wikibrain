...
/**
 * Created by Josh on 12/3/14.
 */

public class LocalPageAutocompleteSqlDao extends AbstractSqlDao<LocalPage> implements LocalPageDao {
    private volatile TLongIntHashMap titlesToIds = null;
    private RedirectSqlDao redirectSqlDao;

    public LocalPageAutocompleteSqlDao(WpDataSource dataSource) throws DaoException {
        this(dataSource, true);
    }

    public LocalPageAutocompleteSqlDao(WpDataSource dataSource, boolean followRedirects) throws DaoException{
    ...
    public List<LocalPage> getBySimilarTitle(Title title, NameSpace nameSpace, LocalLinkDao linkDao) throws DaoException {
        DSLContext context = getJooq();
        try {
            List<Result<Record>> results = context.select().
                    from(Tables.LOCAL_PAGE).
                    where(Tables.LOCAL_PAGE.TITLE.startsWith(title.getCanonicalTitle())).
                    and(Tables.LOCAL_PAGE.LANG_ID.eq(title.getLanguage().getId())).
                    and(Tables.LOCAL_PAGE.NAME_SPACE.eq(NameSpace.ARTICLE.getArbitraryId())).
                    limit(100).
                    fetchMany();

            Result<Record> records = results.get(0);
            Iterator<Record> it = records.iterator();

            LinkedHashMap<Number, LocalPage> sortedRecords = new LinkedHashMap<Number, LocalPage>();

            while (it.hasNext()) {
                Record r = it.next();
                LocalPage page = buildLocalPage(r);
                Iterator<LocalLink> links = linkDao.getLinks(page.getLanguage(), page.getLocalId(), false).iterator();
                int count = 0;
                while (links.hasNext()) {
                    count++;
                    links.next();
                ...
                sortedRecords.put(count, page);
            }

            ArrayList<LocalPage> pages = new ArrayList<LocalPage>(sortedRecords.values());
            while (pages.size() > 10) {
    ...
    public static class Provider extends org.wikibrain.conf.Provider<LocalPageDao> {
        ...
        public Class getType() {
            return LocalPageAutocompleteSqlDao.class;
        }

        @Override
        public String getPath() {
            return "dao.localPageAutocomplete";
        }
        ...
        public LocalPageDao get(String name, Config config, Map<String, String> runtimeParams) throws ConfigurationException {
            ...
            try {
                LocalPageAutocompleteSqlDao dao = new LocalPageAutocompleteSqlDao(
                        getConfigurator().get(
                                WpDataSource.class,
